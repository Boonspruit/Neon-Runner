...
function rebuildTrailMesh(entity) {
  ...
  for (let i = 0; i < points.length; i += 1) {
    ...
    if (len < 0.0001) {
      const d = DIR[entity.dirIndex];
      tx = d.x; tz = d.z; len = 1;
    }

    const nx = (-tz / len) * halfWidth;
    const nz = (tx / len) * halfWidth;

    // Thin glass sheet strip: left-bottom, right-bottom, left-top, right-top
    positions.push(points[i].x + nx, baseY, points[i].z + nz);
    positions.push(points[i].x - nx, baseY, points[i].z - nz);
    positions.push(points[i].x + nx, topY, points[i].z + nz);
    positions.push(points[i].x - nx, topY, points[i].z - nz);
